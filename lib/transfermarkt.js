...
function extractDate(cells, tr) {
  const candidates = [];
  cells.each((_, cell) => candidates.push(clean(tr(cell).text())));
  const text = candidates.join(" | ");
  const match = text.match(/\b(\d{1,2}[./-]\d{1,2}[./-]\d{2,4}|\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{4})\b/i);
  return match?.[1] || "";
}
...
export async function scrapeLatestTransfers() {
  ...
  if (!res.ok) throw new Error(`Transfermarkt alınamadı: ${res.status}`);
  ...
  $("table.items > tbody > tr").each((_, row) => {
    ...
    const position = clean(tr.find("table.inline-table tr").eq(1).find("td").last().text());
    ...
    const marketValue = clean(tr.find("td.rechts").filter((__, cell) => /€/.test($(cell).text())).first().text());

    const allText = cells.map((__, cell) => clean($(cell).text())).get().join(" | ");
    const dateMatch = allText.match(/\b(\d{1,2}[./-]\d{1,2}[./-]\d{2,4}|\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{4})\b/i);
